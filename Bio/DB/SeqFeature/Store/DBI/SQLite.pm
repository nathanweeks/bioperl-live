...
create index index_feature_location on feature_location(seqid,start,end);
END
...
sub _finish_bulk_update {
  ...
  for my $table ('feature', $self->index_tables) {
	...
	my $feature_index = $self->_qualify('feature_location');
	if ($table =~ /parent2child$/) {
	    $sth = $dbh->prepare("REPLACE INTO $qualified_table VALUES (?,?)");
	} elsif ($table =~ /$feature_index$/) {
	    $sth = $dbh->prepare("REPLACE INTO $qualified_table (id,seqid,start,end) VALUES (?,?,?,?)");
...
sub index_tables {
    ...
    return (@t,$self->_qualify('feature_location'));
}
...
END
  ...
  my ($seqid,$start,$end,$strand) = $index_flag ? $self->_get_location($object) : (undef)x6;
...
sub bulk_replace {
    ...
    foreach my $object (@objects) {
        my $id = $object->primary_id;
        my (undef,undef,undef,$strand) = $index_flag ? $self->_get_location($object) : (undef)x4;
        my $primary_tag = $object->primary_tag;
        my $source_tag  = $object->source_tag || '';
        $primary_tag    .= ":$source_tag";
        my $typeid   = $self->_typeid($primary_tag,1);
...
sub _dump_store {
  ...
  for my $obj (@_) {
    my $id       = $self->next_id;
    my ($seqid,$start,$end,$strand) = $indexed ? $self->_get_location($obj) : (undef)x4;
...
sub _update_location_index {
    my $self = shift;
    my ($obj,$id) = @_;
    my ($seqid,$start,$end,$strand) = $self->_get_location($obj);

    my $table = $self->_qualify('feature_location');
    $self->_delete_index($table,$id);

	my $sql    = "INSERT INTO $table (id,seqid,start,end) values (?,?,?,?)";
	my @args   = ($id,$seqid,$start,$end);
